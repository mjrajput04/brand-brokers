...
import { ChevronLeft, ChevronRight, Info, Share2, X, ArrowRight } from "lucide-react";
...
export default function RosterView({ items }: { items: any[] }) {
  ...
  if (!creators.length) {
    return <section id="roster" className="section-wrap roster-section" />;
  }
          ...
          <div style={{ overflow: "hidden", width: "100%" }} className="px-12 md:px-16">
            <div className="flex items-center justify-center gap-2 md:gap-4 w-full">
              {indices.map((ci, pos) => {
                ...
                const Icon = getIcon(creator.icon);
              ...
              <div className="p-5 sm:p-8">
                <div className="flex items-center gap-4 sm:gap-5 mb-6 sm:mb-8">
                  <div
                    ...
                    style={{ background: "var(--t-icon-bg)", border: "1px solid var(--t-card-border)" }}
                  >
                    {(() => { const MIcon = getIcon(selectedCreator.icon); return <MIcon className="w-8 h-8" style={{ color: "var(--t-text)" }} />; })()}
                  </div>
                  <div>
