...
export default function Problem() {
  ...
  const scanOpacity = isDark ? "0.06" : "0.15";
  const shimmerColor = isDark ? "rgba(255,255,255,0.7)" : "rgba(0,0,0,0.2)";
  ...
  return (
    <section id="problem" className="section-wrap problem-section" style={{ position: "relative", paddingTop: "clamp(3rem,6vw,5rem)", paddingBottom: "clamp(3rem,6vw,5rem)" }}>
      <div aria-hidden style={{ position: "absolute", inset: 0, pointerEvents: "none", backgroundImage: `linear-gradient(rgba(${isDark ? "255,255,255" : "0,0,0"},${gridOpacity}) 1px,transparent 1px),linear-gradient(90deg,rgba(${isDark ? "255,255,255" : "0,0,0"},${gridOpacity}) 1px,transparent 1px)`, backgroundSize: "56px 56px", maskImage: "radial-gradient(ellipse at 50% 50%,black 30%,transparent 75%)" }} />
      <div aria-hidden style={{ position: "absolute", left: 0, right: 0, top: `${scan}%`, height: 1, background: `linear-gradient(90deg,transparent,rgba(${isDark ? "255,255,255" : "0,0,0"},${scanOpacity}) 40%,rgba(${isDark ? "255,255,255" : "0,0,0"},${scanOpacity}) 60%,transparent)`, pointerEvents: "none" }} />
      ...
      <div className="section-inner" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div className="reveal flex flex-col gap-2">
          {rows.map((r, i) => {
            ...
            return (
              ...
              >
                <div style={{ height: 1, background: on ? `linear-gradient(90deg,transparent,rgba(${isDark ? "255,255,255" : "0,0,0"},${isDark ? "0.18" : "0.1"}),transparent)` : "transparent", transitionProperty: "background", transitionDuration: "0.25s", transitionTimingFunction: "ease" }} />
                ...
                <div className="sm:hidden" style={{ padding: "14px 16px" }}>
                  ...
                  <div style={{ height: 4, borderRadius: 99, background: "var(--t-bar-bg)", overflow: "hidden" }}>
                    <div style={{
                      height: "100%", width: on ? `${r.pct}%` : "0%", borderRadius: 99,
                      background: on ? "linear-gradient(90deg,var(--t-text),var(--t-text-muted))" : "var(--t-card-bg2)",
                      transition: "width 1.1s cubic-bezier(.16,1,.3,1)",
                      position: "relative", overflow: "hidden",
                    }}>
                      {on && <div style={{ position: "absolute", inset: 0, background: `linear-gradient(90deg,transparent,${shimmerColor},transparent)`, animation: "shimmer 1.6s linear infinite" }} />}
                    </div>
                  </div>
                </div>
                ...
                <div className="hidden sm:grid" style={{ gridTemplateColumns: "1fr 32px 1fr auto", alignItems: "center", gap: 0 }}>
                  <div style={{ padding: "14px 18px" }}>
                    ...
                    <div style={{ height: 4, borderRadius: 99, background: "var(--t-bar-bg)", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: "100%", background: "var(--t-card-bg2)", borderRadius: 99 }} />
                    </div>
                  </div>
        ...
        <div className="reveal mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-5 rounded-2xl" style={{ background: "var(--t-card-bg)", border: "1px solid var(--t-card-border)" }}>
          ...
          <a
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 22px", borderRadius: 10, fontWeight: 900, fontSize: 12, color: isDark ? "#000" : "#fff", background: isDark ? "linear-gradient(135deg,#fff,#d1d5db)" : "linear-gradient(135deg,#1a1a1a,#444444)", textDecoration: "none", flexShrink: 0, minHeight: 44 }}
